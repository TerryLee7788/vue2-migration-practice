...
用以下訊號判斷，符合兩項以上就算「太多」：
- 改動涉及兩個以上互不相關的關注點（例如同時動到「表單驗證邏輯」「路由設定」「樣式系統」）
- 改動的檔案分屬明顯不同的目錄/模組，彼此沒有直接依賴
- 其中包含「新增依賴／建置工具變更」跟「一般功能程式碼」混在一起
- 有明顯可獨立成立、各自能讀懂的邏輯單元（例如先加一個 util function、再讓元件改用它 ——
  這其實是兩個可以分開審的動作）
...
- 若這個 repo 有 `CLAUDE.md`／`.claude/skills/` 之類的專案慣例（例如這裡的 Vue 3 Options API
  限制、`✅ 遷移點 N` 註解慣例），確認拆分後的每個 commit 訊息與範圍不會違背這些慣例。
